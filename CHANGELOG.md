...
- **Works with every current `@react-native-firebase/messaging`** — v26 removed the namespaced API (`messaging().getToken()`), which made the SDK fail with `Object is not a function` on fresh Firebase installs. The SDK now auto-detects the API shape and speaks both the namespaced (≤ v25) and modular (v26+) forms.
...
### Upgrading
- No action. You may delete the hand-copied `<service>` block and the five telephony `<uses-permission>` entries from your manifest if you prefer — the merger provides them either way.
